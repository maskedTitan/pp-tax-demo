<script>
    ...
    async function createPayPalComponent(checkout, PayPal) {
        ...
        paypalConfig.onAuthorized = (data, actions) => {
            console.log("[PayPal] onAuthorized");
        ...
        paypalConfig.onAdditionalDetails = async (state, component) => {
            ...
            console.log("[PayPal] onAdditionalDetails details:", state.data?.details);
            component.setStatus("loading");
            ...
            if (state.data?.details?.payerID) {
                paypalPayerId = state.data.details.payerID;
            }
            ...
            <div class="space-y-6">
                {#if !paymentSuccess}
                    <CheckoutOptions 
                        ...
                        bind:isRecurring 
                        showZeroDollarAuth={false}
                        bind:useServiceAddress 
                        bind:lockAddress 
                        bind:enableSessionTimeout 
                        {TIMEOUT_SECONDS} 
                        providerName="Adyen config" 
                    />
