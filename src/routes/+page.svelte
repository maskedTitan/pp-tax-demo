	...
	async function initializePayPal() {
		try {
			...
			paypal = await loadScript({
				...
				enableFunding: "venmo",
				"data-namespace":
					"paypal_sdk_" + (isProduction ? "prod" : "sandbox"), // Use namespace to avoid conflicts
			});
	...
	<div class="container mx-auto px-8 max-w-7xl py-8">
		...
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
			...
			<div class="space-y-6">
				<!-- Environment Toggle -->
				<div
					class="bg-white shadow-sm rounded-xl p-4 border border-slate-200 flex justify-between items-center"
				>
					<div class="flex items-center gap-2">
						...
						>
						<span
							class={`px-2 py-1 rounded-md text-xs font-bold ${isProduction ? "bg-blue-100 text-blue-700" : "bg-amber-100 text-amber-700"}`}
						>
					...
					<button
						...
						class={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${isProduction ? "bg-blue-600" : "bg-slate-200"}`}
					>
			...
			<div class="text-center space-y-3">
				{#if !isProduction}
					...
					<p class="text-slate-300 text-sm">
						Use PayPal sandbox credentials to test the checkout.
						Toggle to Production to use real credentials.
					</p>
				{:else}
					<div
						class="inline-flex items-center gap-2 bg-blue-500/20 text-blue-300 px-4 py-2 rounded-full text-sm font-semibold"
					>
						<span>🚀</span> Production Environment - Live Mode
					</div>
					<p class="text-slate-300 text-sm">
						You are in Production Mode. Real payments will be
						processed.
					</p>
				{/if}
			</div>
